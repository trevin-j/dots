...
Skills are modular, self-contained guides that extend Opencode's capabilities by providing
...
│   │   ├── name: (required, mush match directory name)
...
1. Choose the skill location (project or global)
...
### Step 1: Choose the Skill Location

Skills live in one of two places. Ask the user which location to use before doing anything else:
...
Ask for optional `author` and `license` metadata during planning. If provided, include them in the frontmatter.
...
When creating a new skill from scratch, always run the `init_skill.py` script. Only create resource directories (scripts/references/assets) if the user asked for them.
...
After initialization, add only the resource directories the user requested. Do not create placeholder files.
...
Only create resource directories the user asked for. Do not create unused `scripts/`, `references/`, or `assets/` folders.
...
Write the YAML frontmatter with `name` and `description`, plus optional `author` and `license` if the user provides them. Ask for author and license during skill planning so you can include them if desired.

- `name`: The skill name
- `description`: This is the primary triggering mechanism for your skill, and helps Opencode understand when to use it.
...
- `author`: Optional. Include only if the user wants attribution.
- `license`: Optional. Include only if the user wants a license statement.
...
Run the validator to confirm the skill structure if needed:
